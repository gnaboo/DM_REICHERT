...
let crib (main: carte array) (extra: carte) : int =
  ...
  let new_arr = Array.init 5 (fun i -> 
    let el = main_simple.(i) in
    ctr := ctr + el.true_val; 
    added := (el::(!added));

    if !ctr = 15 then (
      List.iter (fun el -> if Hashtbl.find dupl el > 1 then incr dupl_in_15) (!added); 
      added := []; 
      0
    ) 
  ...
  let sum = Hashtbl.fold (fun _ value acc -> acc + value) 0 tbl in
