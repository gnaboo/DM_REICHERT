...
let idem tab = 
  ...
  Array.iteri (fun i el -> 
      ...
      if !bool && i <> !curr then bool := false) tab;
  !bool;; 
...
let rec zeroreckt n = if n = 0 then 0 else (n/5) + zeroreckt (n/5);;


type 'a arbre_arite_qlc = Vide | Noeud of ('a) * ('a arbre_arite_qlc list);;


let rec hauteurarbre arbre = match arbre with
  | Vide -> 0
  | Noeud(_, liste) -> let l = ref [] in List.iter (fun el -> l := (hauteurarbre el)::!l) liste; (List.fold_left (max) 0 !l) + 1;;


let test =   
  for i = 0 to 52 do 
    Printf.printf "%d! = fact:%d | zero:%d\n" i (fact i) (zerofact2 i);
    Printf.printf "%d! = fact:%d | zerorec:%d\n" i (fact i) (zeroreckt i);
  done;
  ();;
